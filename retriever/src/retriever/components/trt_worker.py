...
class TRTWorkerConfig(BaseModel):
    """Configuration for the TRTWorker service including model and HTTP server settings."""
...
class TrtWorker:
    ...
    def __init__(self):
        ...
        trt_worker_config = TRTWorkerConfig.model_validate(
            config.get(self.__class__.__name__, {})
        )
        self._prefill_queue_nats_server = os.getenv(
            "NATS_SERVER", "nats://localhost:4222"
        )
        self._prefill_queue_stream_name = trt_worker_config.served_model_name
        logger.info(
