...
class Processor:
    ...
    worker = depends(TrtWorker)
    # router = depends(Router)

    def __init__(self):
        config = ServiceConfig.get_instance()
        processor_config = ProcessorConfig.model_validate(
            config.get(self.__class__.__name__, {})
        )
        # class_name = self.__class__.__name__
        # self.engine_args = parse_vllm_args(class_name, "")
        # self.model_config = self.engine_args.create_model_config()
        self.tokenizer = self._create_tokenizer(model_path=processor_config.model)
    ...
    def _create_tokenizer(self, model_path: str) -> AnyTokenizer:
    ...
    async def async_init(self):
        ...
        comp_ns, comp_name = TrtWorker.dynamo_address()  # type: ignore
        self.worker_client = (
        ...
        # # kv_listener = runtime.namespace("dynamo").component("TrtWorker")
    ...
    @dynamo_endpoint(name="embeddings")
    async def embeddings(self, raw_request: ChatCompletionRequest):
