...
# from retriever.common.parser import LLMAPIConfig
...
# )
from retriever.common.utils import ConversationMessage
...
class ChatProcessorMixin:
    def __init__(self, engine_config: Any, using_engine_generator: bool = False):
...
class BaseChatProcessor:
    ...
    def _get_role(self, request: Any) -> str:
    ...
    def _stream_usage_info(
        self, request: Any, prompt_tokens: int, completion_tokens: int
    ):
        if (
    ...
    def _create_logprobs(self, token_ids: List[int], logprobs: List[float]) -> Any:
